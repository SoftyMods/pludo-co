...
def display_company_products(companies):
    if len(companies) > 0:
        ...
        company_choice = get_valid_input(1, len(companies), "Enter the number of the company you want to view products/projects/cars for: ")
        company = companies[company_choice - 1]
        if len(company['products']) == 0:
            print(f"{company['name']} has no products/projects/cars.")
        else:
            print("\nProducts/projects/cars:")
            for i, product in enumerate(company['products'], 1):
                print(f"{i}. {product['name']} - Investment: ${product['investment']}")
    else:
        print("You don't have any companies to view products/projects/cars for. Create a company first.")
        
def add_new_product_to_company(companies, cash_balance):
    if len(companies) > 0:
        display_companies_list(companies)
        company_choice = get_valid_input(1, len(companies), "Enter the number of the company you want to add a product/project/car to: ")
        company = companies[company_choice - 1]

        product_name = input("Enter a name for your new product/project/car: ")
        investment = get_valid_input(0, float('inf'), "Enter the amount of money you want to invest in this product/project/car (in dollars): ")

        if cash_balance >= investment:
            cash_balance -= investment
            company['products'].append({'name': product_name, 'investment': investment})
            print(f"You have successfully added {product_name} to {company['name']} with an investment of ${investment}.")
        else:
            print("You don't have enough cash to invest in this product/project/car. Please try again.")
    else:
        print("You don't have any companies to add a product/project/car to. Create a company first.")
    return companies, cash_balance

def remove_product_from_company(companies):
    if len(companies) > 0:
        display_companies_list(companies)
        company_choice = get_valid_input(1, len(companies), "Enter the number of the company you want to remove a product/project/car from: ")
        company = companies[company_choice - 1]

        if len(company['products']) == 0:
            print(f"{company['name']} has no products/projects/cars to remove.")
        else:
            print("Products/projects/cars:")
            for i, product in enumerate(company['products'], 1):
                print(f"{i}. {product['name']} - Investment: ${product['investment']}")
            product_choice = get_valid_input(1, len(company['products']), "Enter the number of the product/project/car you want to remove: ")

            removed_product = company['products'].pop(product_choice - 1)
            print(f"{removed_product['name']} has been removed from {company['name']}.")
    else:
        print("You don't have any companies to remove a product/project/car from. Create a company first.")
    return companies
...
def main_game_loop(player_name, cash_balance, companies, business_types, management_personnel, offshore_locations, difficulty_level):
    ...
    while True:
        ...
        print("9. View products/projects/cars")
        print("10. Add a product/project/car to a company")
        print("11. Remove a product/project/car from a company")
        print("12. Quit game")
        
        choice = get_valid_input(1, 12, "Enter the number corresponding to your choice: ")
